...
class ChecklistCubit extends Cubit<ChecklistState> {
  ...
  Future<void> getChecklist(
    ...
    try {
      var checkLists = await _checklistRepo.getChecklist(
          title: title,
  ...
  Future<void> updateChecklistStatus(
      {required String id, bool? status, String? assigneeId}) async {
    emit(UpdateChecklistLoadingState());
    try {
      var checkList = await _checklistRepo.updateChecklistStatus(
          id: id, status: status, assigneeId: assigneeId);
